...
# Assume current working directory is set to python
...
try:
    ...
    print(f"Python path error:", err)
    files = ", ".join([f"'{p.name}'" for p in Path(".").glob("*")])
    print("Files:", files)
    raise RuntimeError(f"Python path error at {files}: {err}")
...
def init_simulated_system(n_species, grid_size, cell_capacity):
    ...
    sim_file = cn.init_simulated_system(out_dir=folder_path, n_species=n_species,
                                        grid_size=grid_size, cell_capacity=cell_capacity)
    # Example sim_file: {out_dir}/pickles/init_cell_4582_c20_s5_d0.3_t0.25.pkl
...
def run_policy(sim_file, run, init):
    ...
    for progress in cn.run_policy_generator(rnd_seed=run["rnd_seed"],
                                            ...
                                            # recurrent monitoring, dynamic protection
                                            observePolicy=run["observePolicy"],
                                            rewardMode=run["rewardMode"],  # objective: minimize species loss
                                            # wd=run["simDataDir"],  # directory with pre-simulated systems
                                            # resolution=np.array([5, 5]), # need to be divisors of grid size
                                            resolution=np.array([run["resolution"], run["resolution"]]),
                                            wd=None,
